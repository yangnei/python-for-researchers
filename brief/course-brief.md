...
- **Learning profile:** Strong, disciplined self-learner; comfortable with abstraction,
  research methods, and reading dense material. Can absorb concepts faster than the typical
  beginner, so the pace is **accelerated** and the hand-holding is reduced.
...
## Format & Duration
- **~20 hours total**, delivered as **10 two-hour sessions** (+ an optional 11th capstone session).
- Each session is self-contained: Concept → Live Example → Practice → (break) → more Practice → Traps recap → Summary.
...
## Scope of topics
The course spans the standard introductory-Python topic set: functions & variables,
conditionals, loops, exceptions, libraries, unit tests, file I/O, regular expressions,
object-oriented programming, and the "Pythonic" power-tools (comprehensions, generators,
`*args`/`**kwargs`, type hints). All lesson prose, slides, examples, and cheat sheets are
**original**, written for this learner.
